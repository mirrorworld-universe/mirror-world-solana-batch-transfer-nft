...
class CandyMachineNotLiveError extends CandyMachineError {
  constructor(goLiveDate, options) {
    super({
      ...
      problem: `You're trying to mint from a Candy Machine which is not live yet. ` + (goLiveDate ? `It will go live on ${DateTime.formatDateTime(goLiveDate)}.` : `Its live date has not been set yet.`),
...
class CandyMachineEndedError extends CandyMachineError {
  constructor(endSetting, options) {
    const endSettingType = endSetting.endSettingType === mplCandyMachine.EndSettingType.Amount ? 'Amount' : 'Date';
    const endSettingExplanation = endSetting.endSettingType === mplCandyMachine.EndSettingType.Amount ? `All ${endSetting.number} items have been minted.` : `It ended on ${DateTime.formatDateTime(endSetting.date)}.`;
    super({
      options,
